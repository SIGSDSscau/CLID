...
def matrix2flat2(matrix, seq_lens):#由于window num和原来seq_lens的关系，故做了1:seq_lens[i]+1的处理，但是没有适配window——size的关系
    if matrix is None:
        return None

    flat_x = torch.cat(
        [matrix[i][1:seq_lens[i]+1] for i in range(0, len(seq_lens))], dim=0)

    return flat_x

def matrix2flat3(matrix, seq_lens):#用于windowsize = 1, window与token数量对应 无需作特殊处理
    if matrix is None:
        return None
